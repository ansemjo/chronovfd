...
# reset datetime at start
rtc.datetime = time.struct_time((2020, 1, 1, 0, 0, 0, -1, -1, -1))
...
lcd.set_fast_backlight_rgb(*white)
...
def fm_tune_status(cancel = False, clear = False):
  ...
  antc =  response[7] # antenna tuning capacitor value
  antcap = antc
  freque = freq
  return done, freq, valid, band, rssi, snr, antc
...
def wait_for_seek(log = False):
  lastfreq = 0
  while True:
    done, freq, valid, band, rssi, snr, antcap = fm_tune_status()
    if log and freq != lastfreq:
      print(f"\33[2K\r· {(freq/100):6.2f} MHz [{rssi:3d} dBµV, SNR: {snr:3d}, cap: {antcap}] {'ok' if valid else ''}", end="")
      lcd.set_cursor(0, 0)
      lcd.write(f"{(freq/100):6.2f}   {rssi:2d}u {snr:2d}d")
      lastfreq = freq
    if done: break
    time.sleep(0.01)
  if log: print()
...
def full_frequency_scan(print_above = 5):
  ...
  for freq in range(8750, 10790, 5):
    ...
    write_command(FM_TUNE_FREQ, 0x00, freq >> 8, freq & 0xff, 0x00, wait=False)
    time.sleep(0.1)
    done, freq, valid, band, rssi, snr, antcap = wait_for_seek(log = False)
    bar = "#" * snr
    if snr > best[0]: best = (snr, freq)
    if snr > print_above:
      print(f"· {(freq/100):6.2f} MHz [{rssi:3d} dBµV, SNR: {snr:3d}, cap: {antcap}] {'ok' if valid else '  '} {bar}")
      lcd.set_cursor(0, 0)
      lcd.write(f"{(freq/100):6.2f}   {rssi:2d}u {snr:2d}d")
  return best[1]
...
write_command(SET_PROPERTY, 0x00, 0x15, 0x03, 0x22, 0x22) # default: 0x1111
...
known = [ NDR_2, DLF, NDR_903, NDR_INFO, DELTA, ENERGY, KLASSIK, RSH_SUED, RSH_LA, RADIOHH, BYTEFM, DELTA_LA, ROCKHH ]
knowni = 7 # RSH

full_frequency_scan(0) # show all with antcap values

# tune to a specific station
tune(known[knowni])

# frequency sweep to find a good station
#freq = full_frequency_scan()
#tune(freq)

# or just seek to next good channel
#seek()
...
last_seek = ticks_ms()
last_rdstime = ticks_ms()
last_clockface = ticks_ms()
is_red = False
...
while True:
  now = ticks_ms()
  if now < last: # wrapped around
    wrap = 2**29
    last_seek = wrap - last_seek
    last_rdstime = wrap - last_rdstime
    last_clockface = wrap - last_clockface
  last = now

  # if last clock sync is more than 10 minutes ago, show red
  if not is_red and (now - last_seek) > 60000 and (now - last_rdstime) > 600000:
    is_red = True
    lcd.set_fast_backlight_rgb(*redish)

  # update clock and rssi every second
  if (now - last_clockface) > 500:
    last_clockface = now
    lcd.set_cursor(0, 1)
    lcd.write(clock())
    smute, valid, pilot, rssi, snr = fm_status()
    lcd.set_cursor(9, 0)
    lcd.write(f"{rssi:2d}u {snr:2d}d")
    print(f"\r· {(freque/100):6.2f} MHz [{rssi:3d} dBµV, SNR: {snr:3d}, cap: {antcap}] {'ok' if valid else '  '}", end="")

  # check button or time to re-seek
  if not btn.value or ((now - last_rdstime) > 300000 and (now - last_seek) > 300000):
    print()
    is_red = False
    last_seek = now
    lcd.set_fast_backlight_rgb(*blueish)
    reset_station()
    knowni = (knowni + 1) % len(known)
    tune(known[knowni])
    #seek()
    time.sleep(1)
