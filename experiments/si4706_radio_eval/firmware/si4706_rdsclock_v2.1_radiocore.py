...
p = neopixel.NeoPixel(board.IO4, 1)
p.brightness = 0.5
p.fill((0, 0, 0))
...
vfd_device = I2CDevice(i2c, 0x42)
def display(text):
  with vfd_device as vfd:
    vfd.write(text)
...
def full_frequency_scan(print_above = 5):
  ...
  for freq in range(8750, 10790, 5):
    ...
    if snr > print_above:
      print(f"· {(freq/100):6.2f} MHz [{rssi:3d} dBµV, SNR: {snr:3d}, cap: {antcap:3d}] {'ok' if valid else '  '} {bar}")
  return best[1]
