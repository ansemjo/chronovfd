...
static esp_err_t set_config_handler(const wifi_prov_config_set_data_t *req_data, wifi_prov_ctx_t **ctx)
{
    ...
    ESP_LOGI(TAG, "WiFi credentials received for SSID %s", req_data->ssid);
                                   ...
                                   int event_id, void* event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
         ...
         * Schedule timer to stop provisioning app after 15 seconds. */
        g_prov->wifi_state = WIFI_PROV_STA_CONNECTED;
        if (g_prov && g_prov->timer) {
            esp_timer_start_once(g_prov->timer, 15000*1000U);
        }
...
esp_err_t app_prov_is_provisioned(bool *provisioned)
{
    *provisioned = false;
    ...
    if (strlen((const char*) wifi_cfg.sta.ssid)) {
        *provisioned = true;
        ESP_LOGI(TAG, "Found credentials for SSID: %s", (const char*) wifi_cfg.sta.ssid);
